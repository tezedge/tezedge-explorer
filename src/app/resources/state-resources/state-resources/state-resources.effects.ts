...
import { EMPTY, Observable, ObservedValueOf, of, tap, throwError } from 'rxjs';
...
export class StateResourcesEffects {
  ...
  stateResourcesLoad$ = createEffect(() => this.actions$.pipe(
    ...
    withLatestFrom(this.store, (action: StateResourcesLoad | StateResourcesClose, state: ObservedValueOf<Store<State>>) => ({
      action,
      state
    })),
  ...
  stateResourcesBlocksLoad$ = createEffect(() => this.actions$.pipe(
    ...
    withLatestFrom(this.store, (action: StateResourcesLoadBlocks | StateResourcesClose, state: ObservedValueOf<Store<State>>) =>
      ({ action, state })
    ),
    switchMap(({ action, state }) => {
      return action.type === STATE_RESOURCES_CLOSE
        ...
        : this.stateResourcesService.getBlockStateResources(state.settingsNode.activeNode.http, action.payload.level)
          .pipe(
            map((blocks: StateResourcesBlockData[]) => {
              const stateResourcesBlockData = blocks.filter(b => b.blockLevel === state.resources.stateResourcesState.blockLevelFilter);
              const activeBlock = stateResourcesBlockData.find(b => b.blockRound === state.resources.stateResourcesState.blockRoundFilter) || stateResourcesBlockData[0];

              return ({
                type: STATE_RESOURCES_LOAD_BLOCKS_SUCCESS,
                payload: { activeBlock, blocks }
              });
            }),
            tap((emitted: StateResourcesLoadBlocksSuccess) =>
              this.router.navigate(['resources', 'state', emitted.payload.activeBlock.blockLevel, emitted.payload.activeBlock.blockRound])
            ),
            catchError(err => throwError(err))
          );
    }),
  ...
  private catchError = (error, initiator: string): Observable<ErrorAdd> => of({
    type: ADD_ERROR,
