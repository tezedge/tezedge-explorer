 ...
 * @throws RpcError
 */
export const rpc = (selector) => (source) => source.pipe(
//exec callback function
map(state => (Object.assign(Object.assign({}, state), { rpc: selector(state) }))), flatMap((state) => {
    if (!state.rpc.payload && state.ws && state.ws.webSocket && !state.ws.webSocket.closed) {
        ...
        return state.ws.webSocket.asObservable().pipe(filter(response => response.id === id), map(response => (Object.assign(Object.assign({}, state), { [state.rpc.path]: response.result }))));
    }
    return state.rpc.payload !== undefined ?
        ...
        // use only response
        map(event => (Object.assign(Object.assign({}, state), { [state.rpc.path]: event.response }))), 
        // catchError
        catchError(error => {
            ...
            return throwError(Object.assign(Object.assign({}, error), { state }));
        }))
        :
            // get
