...
export class MempoolBlockApplicationEffects {
  ...
  mempoolBlockApplicationDetailsLoadSuccess$ = createEffect(() => this.actions$.pipe(
  ...
  mempoolBlockApplicationDetailsLoad$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap(({ action, state }) =>
      this.mempoolService.getBlockRounds(state.settingsNode.activeNode.http, action.payload.level, action.payload.round)
    ),
