...
import {
  ...
  MEMPOOL_PREENDORSEMENT_SET_ROUND,
...
export class MempoolPreEndorsementComponent implements OnInit, OnDestroy {
  ...
  private listenToBlockChange(): void {
    this.store.select(selectMempoolConsensusActiveBlockLevel).pipe(
      ...
      this.store.dispatch<MempoolPreEndorsementLoad>({ type: MEMPOOL_PREENDORSEMENT_LOAD, payload: { blockLevel } })
    );
    this.store.select(selectMempoolConsensusActiveBlockRound).pipe(
      ...
      this.store.dispatch<MempoolPreEndorsementSetRound>({ type: MEMPOOL_PREENDORSEMENT_SET_ROUND, payload: { round } });
    });
  }
