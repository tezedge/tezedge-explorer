...
import { locale } from 'moment-mini-ts';
...
export class TezedgeTimeValidator {
  ...
  static getDateFormat(value: string): Date {
    ...
    const date = new Date(Date.parse(TezedgeTimeValidator.getValidDate(dateStr)));
    const hms = value.slice(0, value.indexOf(value.includes('.') ? '.' : ','));

    date.setHours(Number(hms.slice(0, hms.indexOf(':'))));
  ...
  private static getValidDate(dateStr): string {
    return ['DD MMM YY', 'DD/MM/YY', 'DD-MM-YY', 'DD.MM.YY']
