...
} from '@mempool/consensus/preendorsements/mempool-pre-endorsement/mempool-pre-endorsement.actions';
import { MempoolPreEndorsementService } from '@mempool/consensus/preendorsements/mempool-pre-endorsement/mempool-pre-endorsement.service';
...
export class MempoolPreEndorsementEffects {
  ...
  //   withLatestFrom(this.store, (action: MempoolPreEndorsementSetRound, state: ObservedValueOf<Store<State>>) => ({ action, state })),
  ...
  mempoolPreEndorsementLoad$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap(({ action, state }) => {
      const blockLevel = (action as MempoolPreEndorsementLoad).payload?.blockLevel;
      if (action.type === MEMPOOL_PREENDORSEMENT_STOP || !blockLevel) {
        return EMPTY;
      }
      return this.preEndorsementService.getPreEndorsingRights(state.settingsNode.activeNode.http, blockLevel);
    }),
