...
import { calculateViewDimensions, id } from '@swimlane/ngx-charts';
...
@Component({
  selector: 'app-tezedge-diagram',
  templateUrl: './tezedge-diagram.component.html',
  styleUrls: ['./tezedge-diagram.component.scss'],
...
export class TezedgeDiagramComponent extends GraphComponent {

  @Input() clusterHeight = 100;
  // @Input() nodes: Node[] = [];
  ...
  createGraph(): void {
    ...
    const initializeNode = (n: Node) => {
      ...
      if (!n.dimension) {
        ...
        n.dimension = {
          width: n.dimension.width ? n.dimension.width : this.nodeWidth ? this.nodeWidth : 30,
          height: n.dimension.height ? n.dimension.height : this.nodeHeight ? this.nodeHeight : 30
        };
        n.meta.forceDimensions = n.meta.forceDimensions === undefined ? true : n.meta.forceDimensions;
      }
      n.position = {
        x: n.position?.x ? n.position.x : 0,
        y: n.position?.y ? n.position.y : 0
      };
  ...
  tick() {
    ...
    (this.graph.clusters || []).map(n => {
      n.transform = `translate(${n.position.x - n.dimension.width / 2 || 0}, ${
        n.position.y - (n.data.customHeight ?? n.dimension.height) / 2 || 0
      })`;
      if (!n.data) {
        n.data = {};
      }
