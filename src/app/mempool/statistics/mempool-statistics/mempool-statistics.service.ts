...
export class MempoolStatisticsService {
  ...
  getOperationNodeStats(api: string, operationHash: string): Observable<MempoolStatisticsOperation[]> {
    const url = `${api}/dev/shell/automaton/mempool/operation_stats${operationHash ? `?hash=${operationHash}` : ''}`;
