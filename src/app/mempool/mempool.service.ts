...
export class MempoolService {
  ...
  getEndorsementStatusUpdates(): Observable<MempoolEndorsement[]> {
    return this.http.get<any>('http://prechecker.mempool.tezedge.com:18732/dev/shell/automaton/endorsements_status').pipe(
      // return of(this.getMockUpdatesEverySecond()).pipe(
      map(response => {
        return Object.keys(response).map(key => ({
          slot: response[key].slot,
          status: response[key].state,
          receiveTime: response[key].received_time || 0,
          applyTime: response[key].applied_time || 0,
          decodeTime: response[key].decoded_time || 0,
          precheckTime: response[key].prechecked_time || 0,
          broadcastTime: response[key].broadcast_time || 0,
          totalTime: (response[key].received_time || 0) + (response[key].decoded_time || 0) + (response[key].applied_time || 0) + (response[key].prechecked_time || 0) + (response[key].broadcast_time || 0)
        }));
      })
    );
  }

  getEndorsements(blockHash: string, level: number): Observable<MempoolEndorsement[]> {
    this.endorsementUpdates = mockUpdates;
    const url = 'http://prechecker.mempool.tezedge.com:18732/dev/shell/automaton/endorsing_rights?block=' + blockHash + '&level=' + level;
    return forkJoin(
      this.http.get<{ [p: string]: number[] }>(url).pipe(
  ...
  private mapGetEndorsements(response: { [p: string]: number[] }): MempoolEndorsement[] {
    const endorsements: MempoolEndorsement[] = [];
    Object.keys(response).forEach(key =>
      response[key].forEach(slot =>
        endorsements.push({ slot, baker: key })
      )
    );
    return endorsements;
  }

  // private getMockUpdatesEverySecond(): MempoolEndorsement[] {
  //   const newSlotUpdateId = Math.floor(Math.random() * 256);
  //   if (this.endorsementUpdates.some(e => e.slot !== newSlotUpdateId)) {
  //     const random = Math.random();
  //     const status = random < 0.26 ? 'pending' : random < 0.5 ? 'prechecked' : random < 0.75 ? 'applied' : 'broadcast';
  //     this.endorsementUpdates = [...this.endorsementUpdates, {
  //       slot: newSlotUpdateId,
  //       status,
  //       totalTime: 0,
  //       broadcastTime: 0,
  //       decodeTime: 0,
  ...
  //     }];
  //   }
  //   return this.endorsementUpdates.map(x => ({ ...x }));
  // }


//   getEndorsements(blockHash: string, level: number): Observable<MempoolEndorsement[]> {
//     this.endorsementUpdates = mockUpdates;
//     const blockHashMock = 'BLi1gsVDJLD81rvsF9rWRB9j4x5wBvmhNtzWNHsVXRf8SV4akrn';
//     const levelMock = 1912266;
//     const url = 'http://prechecker.mempool.tezedge.com:18732/dev/shell/automaton/endorsing_rights?block=' + blockHashMock + '&level=' + levelMock;
...
//   }
//
//   private mapGetEndorsements(response: { [p: string]: number[] }): any {
//     const endorsements: MempoolEndorsement[] = [];
//     Object.keys(response).forEach(key =>
//       response[key].forEach(slot =>
//         endorsements.push({ slot, baker: key })
...
//           this.http.get<any>('https://api.tezos-nodes.com/v1/baker/' + endorsement.baker).toPromise().then(baker => {
//             endorsement.baker = baker.name;
//             endorsement.logo = baker.logo_min;
//           })
//         )
//       ).then((observables) => resolve(observables));
//     });
//
// // .subscribe((baker: any) => {
// //   endorsement.baker = baker.name;
